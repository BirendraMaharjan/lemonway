...
use Lemonway\Common\Traits\Requester;
use Lemonway\Common\Utils\Errors;
use Lemonway\Config\Classes;
use Lemonway\Config\I18n;
use Lemonway\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function setLocale() {
		$set_timer  = microtime( true );
		$this->i18n = new I18n();
		add_action( 'init', array( $this->i18n, 'load' ) );
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'lemonway_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'lemonway_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'lemonway' ),
					__FILE__,
					$err
				);
			}
	...
	public function getClasses( string $target_namespace ): array {
		...
		// Errors::writeLog(
		// [
		// 'title'   => __( 'Lemonway classes are not being loaded by Composer\'s Autoloader' ),
