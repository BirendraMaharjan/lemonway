...
class ReturnWarranty extends Dokan {
	...
	public function init() {
		...
		remove_filter( 'woocommerce_my_account_my_orders_actions', array( 'Dokan_RMA_Order', 'request_button' ) );
		remove_filter( 'dokan_my_account_my_sub_orders_actions', array( 'Dokan_RMA_Order', 'request_button' ) );

		// My order list table actions.
		add_filter( 'woocommerce_my_account_my_orders_actions', array( $this, 'requestButton' ), 10, 2 );
		add_filter( 'dokan_my_account_my_sub_orders_actions', array( $this, 'requestButton' ), 10, 2 );
	...
	public function removeClassAction( $hook, $class_name, $method_name, $priority = 10 ) {
		global $wp_filter;

		if ( isset( $wp_filter[ $hook ] ) ) {
			foreach ( $wp_filter[ $hook ]->callbacks[ $priority ] ?? array() as $callback ) {
				if ( is_array( $callback['function'] ) &&
					is_object( $callback['function'][0] ) &&
					get_class( $callback['function'][0] ) === $class_name &&
					$callback['function'][1] === $method_name ) {

					remove_action( $hook, array( $callback['function'][0], $method_name ), $priority );
				}
			}
		}
	}
	...
	public function warrantyTypes( $type ) {
		...
		$type['no_warranty']       = __( 'No Refund', 'lemonway' );
		$type['included_warranty'] = __( 'Refund', 'lemonway' );
	...
	public function requestButton( $actions, $order ) {
		...
		$actions['request_warranty'] = array(
			'url'  => $url,
			'name' => __( 'Request Refund', 'lemonway' ),
		);
		return $actions;
	}
}
